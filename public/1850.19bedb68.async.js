(self.webpackChunkvirtuallab = self.webpackChunkvirtuallab || []).push([
    [1850], {
        10927: function(s) {
            s.exports = {
                equipment_icon_aid_click: "equipment_icon_aid_click___WKQxZ",
                equipment_icon_aid_default: "equipment_icon_aid_default___MPs_B",
                equipment_icon_aid_hover: "equipment_icon_aid_hover___2g-Wr",
                equipment_icon_electro_electrode_click: "equipment_icon_electro_electrode_click___2h8hs",
                equipment_icon_electro_electrode_default: "equipment_icon_electro_electrode_default___5jKBT",
                equipment_icon_electro_eq_click: "equipment_icon_electro_eq_click___GwonY",
                equipment_icon_electro_eq_default: "equipment_icon_electro_eq_default___3WJ2p",
                equipment_icon_electro_gas_click: "equipment_icon_electro_gas_click___2QiJ0",
                equipment_icon_electro_gas_default: "equipment_icon_electro_gas_default___J7kJ4",
                equipment_icon_electro_liquid_click: "equipment_icon_electro_liquid_click___3MSYW",
                equipment_icon_electro_liquid_default: "equipment_icon_electro_liquid_default___3JT0d",
                equipment_icon_gas_click: "equipment_icon_gas_click___2jFXd",
                equipment_icon_gas_default: "equipment_icon_gas_default___1_qbw",
                equipment_icon_gas_hover: "equipment_icon_gas_hover___rC-6U",
                equipment_icon_liquid_click: "equipment_icon_liquid_click___N5HLa",
                equipment_icon_liquid_default: "equipment_icon_liquid_default___2urK-",
                equipment_icon_liquid_hover: "equipment_icon_liquid_hover___Y0CsQ",
                equipment_icon_open_default: "equipment_icon_open_default___3_a1a",
                "equipment_icon_put-away_default": "equipment_icon_put-away_default___1SHxy",
                equipment_icon_reactor_click: "equipment_icon_reactor_click___23J6K",
                equipment_icon_reactor_default: "equipment_icon_reactor_default___g5F3B",
                equipment_icon_reactor_hover: "equipment_icon_reactor_hover___1fUdv",
                equipment_icon_solid_click: "equipment_icon_solid_click___1GJ4c",
                equipment_icon_solid_default: "equipment_icon_solid_default___3NrBQ",
                equipment_icon_solid_hover: "equipment_icon_solid_hover___1BqmT",
                icon_keyboard_def: "icon_keyboard_def___23e0M",
                icon_keyboard_hov: "icon_keyboard_hov___1fniq",
                lock: "lock___2rDz_",
                qicaiku_button_back_default: "qicaiku_button_back_default___3OkDA",
                qicaiku_button_back_pressed: "qicaiku_button_back_pressed___3d46F",
                qicaiku_button_default: "qicaiku_button_default___3cCz4",
                qicaiku_button_hover: "qicaiku_button_hover___202IL",
                qicaiku_button_pressed: "qicaiku_button_pressed___urnvD",
                search_icon_line_default: "search_icon_line_default___Y0CHD",
                search_icon_search_Hover: "search_icon_search_Hover___3wL44",
                search_icon_search_default: "search_icon_search_default___BP3rG",
                tanchuang_close: "tanchuang_close___w72aj",
                item: "item___1thvy",
                lockWrap: "lockWrap___18q_Z",
                lockWrap_Second: "lockWrap_Second___Mvwyf",
                hover: "hover___3bZcR",
                img_wrap: "img_wrap___3cryq",
                img: "img___14Shq",
                h5: "h5___1kqNt",
                threeModel: "threeModel___x7jqF",
                eqAnimationLeft: "eqAnimationLeft___3ouY0",
                eqRotate: "eqRotate___3m7X0",
                eqAnimationRight: "eqAnimationRight___3Uqkn"
            }
        },
        31496: function(s) {
            s.exports = {
                list: "list___3QSa_",
                eqItem: "eqItem___3MESf",
                wrapAnimation: "wrapAnimation___3XXOS",
                wrapRotate: "wrapRotate___32gM4",
                eqRotate: "eqRotate___AcVp1"
            }
        },
        58951: function(s) {
            s.exports = {
                ScrollBar: "ScrollBar___1sQ2U"
            }
        },
        21750: function(s, u, e) {
            "use strict";
            var d = e(89032),
                L = e(15746),
                h = e(30929),
                j = e(11729),
                W = e(67294),
                A = e(10927),
                _ = e.n(A),
                i = e(85893),
                Z = m => {
                    var p = m.data,
                        C = m.dataIndex,
                        H = m.showLock,
                        U = m.disabled,
                        K = m.boomStart,
                        N = m.parentStyle,
                        T = m.equipmentLineColumn,
                        F = T === void 0 ? !1 : T,
                        G = p.bgImgUrl,
                        y = p.isLock,
                        J = p.isFreeEq,
                        I = p.iconPngData,
                        R = p.is3d,
                        n = R === void 0 ? !1 : R,
                        c = o => {
                            h.Z.isSDK || (o.target.manunalToggleLogin = !0)
                        },
                        M = ["Left", "Right"],
                        k = (0, i.jsxs)("svg", {
                            className: "threeModel",
                            width: "18",
                            height: "10",
                            viewBox: "0 0 18 10",
                            fill: "none",
                            xmlns: "http://www.w3.org/2000/svg",
                            children: [(0, i.jsx)("path", {
                                d: "M0 1C0 0.447715 0.447715 0 1 0H14C16.2091 0 18 1.79086 18 4V9C18 9.55228 17.5523 10 17 10H4C1.79086 10 0 8.20914 0 6V1Z",
                                fill: "url(#paint0_linear_3534_35902)",
                                fillOpacity: "0.9"
                            }), (0, i.jsx)("path", {
                                d: "M6.17866 2.176C6.75466 2.176 7.22666 2.312 7.57866 2.592C7.92266 2.872 8.09866 3.256 8.09866 3.752C8.09866 4.376 7.77866 4.792 7.14666 5C7.48266 5.104 7.74666 5.256 7.92266 5.464C8.11466 5.68 8.21066 5.96 8.21066 6.296C8.21066 6.824 8.02666 7.256 7.65866 7.592C7.27466 7.936 6.77066 8.112 6.14666 8.112C5.55466 8.112 5.07466 7.96 4.71466 7.656C4.31466 7.32 4.09066 6.824 4.04266 6.184H4.99466C5.01066 6.552 5.12266 6.84 5.34666 7.04C5.54666 7.224 5.81066 7.32 6.13866 7.32C6.49866 7.32 6.78666 7.216 6.99466 7.016C7.17866 6.832 7.27466 6.608 7.27466 6.336C7.27466 6.008 7.17066 5.768 6.97866 5.616C6.78666 5.456 6.50666 5.384 6.13866 5.384H5.73866V4.68H6.13866C6.47466 4.68 6.73066 4.608 6.90666 4.464C7.07466 4.32 7.16266 4.104 7.16266 3.824C7.16266 3.544 7.08266 3.336 6.93066 3.192C6.76266 3.048 6.51466 2.976 6.18666 2.976C5.85066 2.976 5.59466 3.056 5.41066 3.224C5.21866 3.392 5.10666 3.648 5.07466 3.992H4.15466C4.20266 3.416 4.41066 2.968 4.79466 2.648C5.15466 2.328 5.61866 2.176 6.17866 2.176ZM9.07953 2.288H11.1675C12.0955 2.288 12.7915 2.544 13.2715 3.056C13.7275 3.536 13.9595 4.232 13.9595 5.144C13.9595 6.048 13.7275 6.744 13.2715 7.232C12.7915 7.744 12.0955 8 11.1675 8H9.07953V2.288ZM10.0155 3.088V7.2H10.9915C11.7035 7.2 12.2235 7.032 12.5515 6.704C12.8715 6.368 13.0315 5.848 13.0315 5.144C13.0315 4.424 12.8715 3.896 12.5515 3.576C12.2235 3.248 11.7035 3.088 10.9915 3.088H10.0155Z",
                                fill: "white"
                            }), (0, i.jsx)("defs", {
                                children: (0, i.jsxs)("linearGradient", {
                                    id: "paint0_linear_3534_35902",
                                    x1: "8.5",
                                    y1: "10",
                                    x2: "8.5",
                                    y2: "1.01331e-07",
                                    gradientUnits: "userSpaceOnUse",
                                    children: [(0, i.jsx)("stop", {
                                        stopColor: "#FF8A58"
                                    }), (0, i.jsx)("stop", {
                                        offset: "1",
                                        stopColor: "#FFE350"
                                    })]
                                })
                            })]
                        }),
                        f = !n || h.Z.isSDK || !h.Z.isLoadZH || window.__testConfig_hide3D;
                    return (0, i.jsx)(L.Z, {
                        "data-testid": "item_col",
                        span: F ? 24 : 12,
                        style: {
                            height: "116px",
                            marginBottom: "10px",
                            pointerEvents: U ? "none" : "auto"
                        },
                        onTouchStart: o => {
                            c(o)
                        },
                        onMouseDown: o => {
                            c(o)
                        },
                        children: (0, i.jsxs)(j.Z, {
                            "data-testid": "item_btn",
                            className: "".concat(_().item, " ").concat(N),
                            hover: _().hover,
                            "data-index": C,
                            children: [y === !1 || J || !H ? null : (0, i.jsx)("div", {
                                "data-testid": "lock-icon",
                                className: "".concat(_().lockWrap, " ").concat(f ? "" : _().lockWrap_Second),
                                children: (0, i.jsx)("div", {
                                    className: _().lock
                                })
                            }), f ? null : (0, i.jsx)("div", {
                                "data-testid": "lock-icon",
                                className: _().lockWrap,
                                children: (0, i.jsx)("span", {
                                    className: _().threeModel,
                                    children: k
                                })
                            }), (0, i.jsx)("div", {
                                className: _().img_wrap,
                                children: (0, i.jsx)("div", {
                                    "data-testid": "boom_start",
                                    className: "".concat(_().img, "  ").concat(K ? _()["eqAnimation".concat(M[Math.round(Math.random())])] : ""),
                                    style: {
                                        background: "url(".concat(G, ") no-repeat -").concat(I.x, "px -").concat(I.y, "px")
                                    }
                                })
                            }), (0, i.jsx)("h5", {
                                className: _().h5,
                                children: p.name
                            })]
                        })
                    })
                };
            u.Z = Z
        },
        61850: function(s, u, e) {
            "use strict";
            e.r(u), e.d(u, {
                default: function() {
                    return R
                }
            });
            var d = e(13062),
                L = e(71230),
                h = e(2824),
                j = e(30929),
                W = e(39428),
                A = e(3182),
                _ = e(67294),
                i = e(58951),
                Z = e.n(i),
                m = e(88922),
                p = e(66127),
                C = e(85893),
                H = n => {
                    var c = n.maxHeight,
                        M = c === void 0 ? 0 : c,
                        k = n.fresh,
                        f = (0, _.useRef)(null),
                        o = (0, _.useRef)(),
                        P = (0, m.Z)(f.current),
                        V = function() {
                            var l = (0, A.Z)((0, W.Z)().mark(function w() {
                                var b, B;
                                return (0, W.Z)().wrap(function(q) {
                                    for (;;) switch (q.prev = q.next) {
                                        case 0:
                                            if (!o.current) {
                                                q.next = 2;
                                                break
                                            }
                                            return q.abrupt("return");
                                        case 2:
                                            return q.next = 4, (0, p.y)();
                                        case 4:
                                            b = f.current, B = new window.IScroll(b, {
                                                click: !1,
                                                preventDefault: !1,
                                                probeType: 2,
                                                scrollbars: !0,
                                                mouseWheel: !0,
                                                interactiveScrollbars: !1,
                                                shrinkScrollbars: "scale",
                                                fadeScrollbars: !0,
                                                useTransform: !0,
                                                useTransition: !0,
                                                bounce: !0
                                            }), o.current = B;
                                        case 7:
                                        case "end":
                                            return q.stop()
                                    }
                                }, w)
                            }));
                            return function() {
                                return l.apply(this, arguments)
                            }
                        }();
                    return (0, _.useEffect)(() => {
                        var l = o.current;
                        l && (l.scrollTo(0, 0), l.refresh())
                    }, [k]), (0, _.useEffect)(() => {
                        var l = o.current;
                        l && l.refresh()
                    }, [P]), (0, _.useEffect)(() => (V(), () => {
                        var l = o.current;
                        l && l.destroy(), o.current = null
                    }), []), (0, C.jsx)("div", {
                        className: Z().ScrollBar,
                        ref: f,
                        style: {
                            maxHeight: M
                        },
                        children: n.children
                    })
                },
                U = H,
                K = e(15559),
                N = e(51035),
                T = e(80125),
                F = e(21750),
                G = e(31496),
                y = e.n(G),
                J = n => {
                    var c = n.list,
                        M = n.addEquipment,
                        k = n.height,
                        f = k === void 0 ? 0 : k,
                        o = n.showLock,
                        P = n.boomStart,
                        V = n.equipmentLineColumn,
                        l = n.sdkModel,
                        w = (0, _.useState)(0),
                        b = (0, h.Z)(w, 2),
                        B = b[0],
                        Q = b[1],
                        q = (0, _.useState)(0),
                        z = (0, h.Z)(q, 2),
                        te = z[0],
                        _e = z[1],
                        $ = (0, _.useRef)(null),
                        D, S, v = 0,
                        ae = 500,
                        Y, E = {
                            x: 40,
                            y: 40
                        },
                        x = {
                            x: 0,
                            y: 0
                        },
                        t, X = document.querySelector("#root"),
                        Oe = null,
                        se = null,
                        ue = 0,
                        pe = null,
                        ge = null,
                        me = 0;
                    (0, _.useEffect)(() => {
                        _e(f)
                    }, [f]), (0, _.useEffect)(() => {
                        X && X.classList[P ? "add" : "remove"](y().wrapAnimation)
                    }, [P]);
                    var ie = r => {
                            var a = r.iconPngData,
                                g = a.w,
                                O = a.h,
                                ee = a.url,
                                se = a.x,
                                ue = a.y;
                            t && t.parentElement && document.body.removeChild(t), t = document.createElement("div"), t.style.background = 'url("'.concat(ee, '") -').concat(se, "px -").concat(ue, "px no-repeat"), t.style.url = "absolute", t.style.width = "".concat(g, "px"), t.style.height = "".concat(O, "px"), t.style.zIndex = "100000", t.style.position = "absolute", t.style.opacity = "0.5", t.style.display = "none", document.body.appendChild(t)
                        },
                        ne = r => {
                            var a = r.target;
                            return a.tagName === "path"
                        },
                        he = r => {
                            var a = r.changedTouches && r.changedTouches[0] || r.touches && r.touches[0] || r;
                            return {
                                x: a && a.pageX || 0,
                                y: a && a.pageY || 0
                            }
                        },
                        be = r => {
                            try {
                                return (0, N.wV)(r.target, y().eqItem)
                            } catch (a) {
                                return null
                            }
                        },
                        ve = (r, a) => {
                            var g = new Date().getTime();
                            return !r || (pe === a || ge === r) && g - me < 500 ? !1 : (pe = a, ge = r, me = g, M(r), !0)
                        },
                        de = r => {
                            var a = be(r),
                                g = he(r);
                            a ? (Oe = a.dataset.index, c[Oe] && (c[Oe].isClickThreeModelIcon = ne(r)), se = g, ue = new Date().getTime()) : (Oe = null, se = null, ue = 0)
                        },
                        we = () => {
                            Oe = null, se = null, ue = 0
                        },
                        ye = r => {
                            if (Oe !== null && se) {
                                var a = be(r),
                                    g = he(r),
                                    O = a ? a.dataset.index : Oe,
                                    ee = c[Oe],
                                    fe = Math.abs(se.x - g.x),
                                    Ce = Math.abs(se.y - g.y),
                                    Ie = new Date().getTime() - ue;
                                (!a || O === Oe) && v === 0 && Ie < 800 && fe < E.x && Ce < E.y && ve(ee, Oe), we()
                            }
                        },
                        oe = (r, a) => {
                            try {
                                var g = be(r);
                                if (r.target.classList !== null && g) {
                                    Y = new Date().getTime(), x = a, v = 0;
                                    var O = g.dataset.index;
                                    Oe = O, se = a, ue = Y, S = c[O], ne(r) ? S.isClickThreeModelIcon = !0 : S.isClickThreeModelIcon = !1, ie(S)
                                }
                            } catch (ee) {
                                x = a, v = 0, t && t.parentElement && document.body.removeChild(t)
                            }
                        },
                        re = (r, a) => {
                            if (v === 0 && (Math.abs(x.y - a.y) > E.y ? v = 2 : x.x - a.x > E.x && t ? (t.style.display = "block", v = 1) : a.x - x.x > E.x && (v = 3)), v === 1 && t) {
                                var g = a.x - E.x,
                                    O = a.y - E.y;
                                t.style.top = "".concat(O, "px"), t.style.left = "".concat(g, "px")
                            }
                        },
                        le = (r, a) => {
                            var g = new Date().getTime() - Y;
                            v === 0 && g < ae && Math.abs(x.y - a.y) < E.y && ve(S, Oe), v === 1 && x.x - a.x > E.x && M(S, a), t && t.parentElement && (document.body.removeChild(t), t = null)
                        };
                    (0, _.useEffect)(() => {
                        var r = $.current;
                        return r && (D && D.destroy(), D = new K.n(r, {
                            start: oe,
                            move: re,
                            end: le
                        }), r.addEventListener("touchstart", de, {
                            passive: !0
                        }), r.addEventListener("touchend", ye), r.addEventListener("touchcancel", we)), () => {
                            r && (r.removeEventListener("touchstart", de), r.removeEventListener("touchend", ye), r.removeEventListener("touchcancel", we)),
                            D && (D.destroy(), D = null), t && t.parentElement && (document.body.removeChild(t), t = null)
                        }
                    }, [$, c]), (0, _.useEffect)(() => {
                        Q(Math.random())
                    }, [c]);
                    var ce = j.Z.isSDK ? l.showEquipmentLock : o;
                    return (0, C.jsx)("div", {
                        className: y().list,
                        ref: $,
                        children: (0, C.jsx)(U, {
                            maxHeight: te,
                            fresh: B,
                            children: (0, C.jsx)(L.Z, {
                                children: c.map((r, a) => (0, C.jsx)(F.Z, {
                                    data: r,
                                    dataIndex: a,
                                    showLock: ce,
                                    boomStart: P,
                                    parentStyle: y().eqItem,
                                    equipmentLineColumn: V
                                }, "_".concat(a.toString())))
                            })
                        })
                    })
                },
                I = n => {
                    var c = n.sdkModel;
                    return {
                        sdkModel: c
                    }
                },
                R = (0, T.$j)(I)(J)
        },
        15746: function(s, u, e) {
            "use strict";
            var d = e(21584);
            u.Z = d.Z
        },
        89032: function(s, u, e) {
            "use strict";
            var d = e(55450),
                L = e.n(d),
                h = e(6999)
        },
        71230: function(s, u, e) {
            "use strict";
            var d = e(92820);
            u.Z = d.Z
        },
        13062: function(s, u, e) {
            "use strict";
            var d = e(55450),
                L = e.n(d),
                h = e(6999)
        }
    }
]);
